# Créditos — La Cucaña Trianera

Documento canónico de créditos y licencias de terceros utilizadas en la
aplicación. La pantalla _Créditos_ dentro del juego refleja este mismo
contenido.

---

## Autoría

- **Diseño, código, arte y dirección:** Luisao
- **Inspiración:** la Cucaña tradicional de la Velá de Santa Ana, Triana
  (Sevilla)
- **Versión:** 1.0.0 — 2026

© 2026 Luisao. Todos los derechos reservados. Ver [LICENSE](LICENSE).

---

## Motor y herramientas

| Componente                | Versión | Licencia      | Enlace                    |
| ------------------------- | ------- | ------------- | ------------------------- |
| Phaser 3                  | 3.90.0  | MIT           | <https://phaser.io>       |
| three.js                  | 0.185.x | MIT           | <https://threejs.org>     |
| Vite                      | 6.3.1   | MIT           | <https://vitejs.dev>      |
| Capacitor (iOS / Android) | 8.x     | MIT           | <https://capacitorjs.com> |
| Sentry (Browser SDK)      | 10.x    | MIT (cliente) | <https://sentry.io>       |
| Terser                    | 5.x     | BSD-2-Clause  | <https://terser.org>      |

Todas las dependencias del proyecto y sus licencias se pueden consultar
ejecutando `npm ls --all` o revisando `package.json` y `package-lock.json`.

---

## Tipografías

Ambas se cargan desde Google Fonts y se distribuyen bajo la **SIL Open Font
License 1.1 (OFL)**, que permite uso libre incluyendo proyectos comerciales
siempre que se reconozca la autoría.

| Fuente         | Autor                      | Licencia | Enlace                                             |
| -------------- | -------------------------- | -------- | -------------------------------------------------- |
| Jersey 10      | Sarah Cadigan-Fried        | OFL 1.1  | <https://fonts.google.com/specimen/Jersey+10>      |
| Press Start 2P | Codeman38 (Cody Boisclair) | OFL 1.1  | <https://fonts.google.com/specimen/Press+Start+2P> |

Texto completo de la OFL: <https://openfontlicense.org/>

---

## Audio

### Efectos de sonido (SFX)

Todos los efectos de sonido del juego son **creaciones originales** del autor,
generados con [jsfxr](https://sfxr.me/) (versión web del generador _sfxr_ de
DrPetter). Las muestras generadas con jsfxr son de **dominio público / CC0**,
y al haberse creado específicamente para este proyecto, forman parte de los
assets propietarios del juego.

Lista de efectos:

- `click.wav` — Pulsación en botones de UI
- `hitHurt.wav` — Impacto / caída del personaje
- `chapuzon.wav` — Caída al río
- `sfx-fail.wav` — Fallo / Game Over
- `sfx-flag.wav` — Cogida de bandera
- `sfx-maxpower.wav` — Impulso perfecto
- `sfx-talk.wav` — Narrador
- `sfx-win.wav` — Victoria
- `victoria.wav` — Fanfarria final
- `intro.wav` — SFX de introducción

### Música

**⚠️ Música del menú principal — pendiente de autorización**

La música que suena en el menú principal es una adaptación a MIDI/BeepBox de
una sevillana del repertorio popular interpretada por _Los del Río_. La
composición original tiene **derechos de autor** y, por tanto, el uso de esta
adaptación queda **pendiente de obtener autorización expresa de los titulares
de los derechos**.

Ver [`docs/email-music-authorization.md`](docs/email-music-authorization.md)
para la plantilla de contacto con la sociedad de gestión y con los titulares.

---

## Assets gráficos

Todos los sprites, fondos, ilustraciones, iconos, capturas y assets visuales
del juego son **creaciones originales** del autor, dibujados en estilo pixel
art a baja resolución, y forman parte de los assets propietarios del juego
bajo la licencia indicada en [LICENSE](LICENSE).

---

## Agradecimientos

- A Triana y a la Velá de Santa Ana por la inspiración.
- A la comunidad de Phaser por el motor y la documentación.
- A jsfxr y a DrPetter por mantener vivo el espíritu del sonido retro.
- A los autores de las tipografías OFL por hacerlas libres.

---

## Contacto

Para cualquier consulta sobre estos créditos, sobre licencias o sobre el uso
de assets en otros proyectos:

**Luisao** — [luisaodeben@gmail.com](mailto:luisaodeben@gmail.com)
