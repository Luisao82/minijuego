...
(`localStorage` del navegador o equivalente en la app nativa) la información
necesaria para que el juego funcione y recuerde tu progreso:
...
servidor. Si desinstalas la aplicación o limpias el almacenamiento del
navegador, esta información se elimina por completo.
...
- **Datos enviados automáticamente al producirse un error:** mensaje y traza
  del error, navegador o sistema operativo, versión de la aplicación,
  pantalla donde se produjo el fallo.
...
- **Finalidad:** servir la versión web de la aplicación y este documento.
- **Datos técnicos registrados por el proveedor:** dirección IP, navegador y
  ruta solicitada, con la única finalidad de servir el contenido y prevenir
  abusos.
...
tienda correspondiente recopilará sus propios datos según sus políticas. La
aplicación en sí no recibe ni accede a esa información.
...
La aplicación es apta para todos los públicos y no recoge datos personales,
por lo que su uso por parte de menores no implica recogida de información
sujeta a normativas como RGPD-K o COPPA.
...
servicios utilizados. La versión vigente siempre será la publicada en la
URL oficial, indicando la fecha de última actualización en la cabecera.
