...
const AREA_LEFT = Math.round(
  (GAME_WIDTH - (VISIBLE_COUNT * CARD_W + (VISIBLE_COUNT - 1) * CARD_GAP)) / 2
)
  ...
  createCarousel() {
    ...
    this.rewards.forEach((reward, i) => {
      ...
      const card = createRewardCard(this, reward, count, {
        ...CARD_LAYOUT,
        onPress: (r, c) => {
          if (!this.detailOpen) this.showRewardDetail(r, c)
        },
      })
