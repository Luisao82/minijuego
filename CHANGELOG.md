...
- **Barcos ambientales del río (`feature/ambient-boats`).** Kayaks, padel-surfistas y galeón Nao Victoria cruzando el río por detrás del barco principal. Configurable en `public/data/ambient-boats.json` (pesos, 3 capas de profundidad con parallax, franjas Y, dirección, animación, click narrativo/URL). Persistente entre partidas, se regenera al volver al menú. Anti-colisión: un barco en una capa no permite otro en dirección opuesta en la misma capa. Uniques (Nao, Andrea, perrete, city-sevilla) alternan dirección entre spawns y nunca duplican. Depth compuesto `layerZ*10000 + y` para que dentro de una capa el más cercano quede delante. Nao Victoria abre `HistoryScene` como historia de prueba; city-sevilla es un tour boat con weight alto. Nuevos ficheros: `src/game/entities/AmbientBoat.js`, `src/game/services/AmbientBoatsService.js`, `src/game/components/ConfirmModal.js`, y assets en `public/assets/boats/`.
- **Banderas del barco principal (`feature/boat-flags`).** Sistema data-driven para banderas de patrocinadores sobre el barco. Configurable en `public/data/boat-flags.json` — `slots` con `(x, y)` en píxeles de la imagen nativa del barco (175×83), `flags` con `{id, file, url, weight, category}`, y `emptyWeight` para probabilidad de slot vacío. Asignación slot→bandera sorteada al entrar en menú (sin repeticiones), persistente entre partidas. Origen central + pre-flip en Sevilla para que el texto quede legible al hacer flip del `gameWorld`. Click → `ConfirmModal` → `openExternalUrl`. Nuevos ficheros: `src/game/entities/BoatFlag.js`, `src/game/services/BoatFlagsService.js`, `public/data/boat-flags.json`, assets en `public/assets/flags/`.
